...
def get_all_image_paths(directory):
    image_paths = []
    for root, _, files in os.walk(directory): # include subfolders
        for file in files:
...
def main(_):
  ...
  sum_mos = 0
  img_dir = "/gpfs/essfs/iat/Tsinghua/liwy/fluxdrive/MagicDriveDiT_gen_crop/"
  score_file_name = "musiq_cores_magicdit_1600x848.txt"
  img_files = get_all_image_paths(img_dir) 
  ...
  with open(score_file_name, 'w') as f:
    for image_path in tqdm(img_files):
      ...
      f.write("{},{:.4f}\n".format(image_path, pred_mos))
      sum_mos += pred_mos
    print('============== Average Precited MOS:', sum_mos / len(img_files))
