...
class DisplayAnyBeauty:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "input": (("*",{})),
                "num_columns":("INT", { "default": 2, "min": 0, "max": 4, "step": 0, }),
            },
        }
    ...
    def execute(self, input,num_columns=2):
        if isinstance(input,list) and (isinstance(input[0],int) or isinstance(input[0],float)):
            table = [input[i:i + num_columns] for i in range(0, len(input), num_columns)]
            if num_columns!=0:
                from tabulate import tabulate
                text = tabulate(table,tablefmt="grid")
            else:
                text = str(input)
        else:
            text = str(input)
...
MISC_CLASS_MAPPINGS = {
    ...
    "DisplayAnyBeauty":DisplayAnyBeauty,
...
MISC_NAME_MAPPINGS = {
    ...
    "DisplayAnyBeauty":"🔧 Display Any Beauty",
